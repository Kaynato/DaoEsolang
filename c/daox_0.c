...
const struct PATH NEW_PATH = { NULL, NULL, NULL, 1, 0, 0, 1, 0, 0 };
...
int main(int argc, char * const argv[])
{
	...
	if (FORCE || ~strcmp(FILE_COMPILED, &inputFileName[strlen(inputFileName)-6]))
	{
		...
		fseek(ifle, 0L, SEEK_END);
		file_size = ftell(ifle);
		
		fseek(ifle, 0L, SEEK_SET);

		/* Cover the file_size with a power of two */
		while ((dao -> prg_allocbits) / 8 < file_size)
			(dao -> prg_allocbits) *= 2;

		/* Allocate data first */
		(dao -> prg_data) = malloc((dao -> prg_allocbits));

		fread((dao -> prg_data), 1, file_size, ifle);
...
#define CELL 			(sizeof(unsigned long) * 8)
