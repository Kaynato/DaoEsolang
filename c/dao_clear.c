...
void swaps(Path path)
{
	...
	if (P_LEN <= CELL)
	{
		...
		unsigned long report = (P_DATA[arrIndex] >> shift) & mask(len);
		unsigned long left = report >> (len >> 1);
		unsigned long right = report & mask(len >> 1);
		unsigned long recombine = (right << (len >> 1)) | left;
...
void reads(Path path)
{
	...
	if (P_LEN < 8)
	{
		String out = bin(report_by_bit_index(path, pos, P_LEN));
